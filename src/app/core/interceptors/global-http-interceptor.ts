...
export class GlobalHttpInterceptor implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    if (req.url.startsWith('/api/')) {
      const apiReq = req.clone({
        url: this.apiUrlService.getApiUrl(req.url)
      });
      const config = getInterceptorConfig(apiReq);
      ...
      if (this.isTokenBasedAuthEntryPoint(req.url)) {
        if (!AuthService.getJwtToken() && !this.authService.refreshTokenPending()) {
          observable$ = this.handleResponseError(apiReq, next, new HttpErrorResponse({error: {message: 'Unauthorized!'}, status: 401}));
        } else if (!AuthService.isJwtTokenValid()) {
          observable$ = this.handleResponseError(apiReq, next, new HttpErrorResponse({error: {refreshTokenPending: true}}));
        } else {
          observable$ = this.jwtIntercept(apiReq, next);
        }
      } else {
        observable$ = this.handleRequest(apiReq, next);
      }
  ...
  private handleResponseError(req: HttpRequest<any>, next: HttpHandler, errorResponse: HttpErrorResponse): Observable<HttpEvent<any>> {
    ...
    if (errorResponse.error && errorResponse.error.refreshTokenPending ||
      errorResponse.status === 401 && !req.url.endsWith('/api/auth/login')) {
      if (errorResponse.error && errorResponse.error.refreshTokenPending ||
