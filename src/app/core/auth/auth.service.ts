...
const tbApiBaseUrl = environment.tbApiBaseUrl;
...
export class AuthService {
  ...
  public login(loginRequest: LoginRequest): Observable<LoginResponse> {
    return this.http.post<LoginResponse>(`${tbApiBaseUrl}/api/auth/login`, loginRequest, defaultHttpOptions()).pipe(
  ...
  public checkTwoFaVerificationCode(providerType: TwoFactorAuthProviderType, verificationCode: number): Observable<LoginResponse> {
    ...
    (`${tbApiBaseUrl}/api/auth/2fa/verification/check?providerType=${providerType}&verificationCode=${verificationCode}`,
  ...
  public publicLogin(publicId: string): Observable<LoginResponse> {
    ...
    return this.http.post<LoginResponse>(`${tbApiBaseUrl}/api/auth/login/public`, publicLoginRequest, defaultHttpOptions());
  }

  public sendResetPasswordLink(email: string) {
    return this.http.post(`${tbApiBaseUrl}/api/noauth/resetPasswordByEmail`,
  ...
  public activate(activateToken: string, password: string, sendActivationMail: boolean): Observable<LoginResponse> {
    return this.http.post<LoginResponse>(`${tbApiBaseUrl}/api/noauth/activate?sendActivationMail=${sendActivationMail}`,
  ...
  public resetPassword(resetToken: string, password: string): Observable<void> {
    return this.http.post<void>(`${tbApiBaseUrl}/api/noauth/resetPassword`, {resetToken, password}, defaultHttpOptions());
  }

  public changePassword(currentPassword: string, newPassword: string, config?: RequestConfig) {
    return this.http.post(`${tbApiBaseUrl}/api/auth/changePassword`, {currentPassword, newPassword}, defaultHttpOptionsFromConfig(config)).pipe(
      tap((loginResponse: LoginResponse) => {
  ...
  public getUserPasswordPolicy() {
    return this.http.get<UserPasswordPolicy>(`${tbApiBaseUrl}/api/noauth/userPasswordPolicy`, defaultHttpOptions());
  }

  public activateByEmailCode(emailCode: string): Observable<LoginResponse> {
    return this.http.post<LoginResponse>(`${tbApiBaseUrl}/api/noauth/activateByEmailCode?emailCode=${emailCode}`,
      null, defaultHttpOptions());
  }

  public resendEmailActivation(email: string) {
    const encodeEmail = encodeURIComponent(email);
    return this.http.post(`${tbApiBaseUrl}/api/noauth/resendEmailActivation?email=${encodeEmail}`,
      null, defaultHttpOptions());
  }

  public loginAsUser(userId: string) {
    return this.http.get<LoginResponse>(`${tbApiBaseUrl}/api/user/${userId}/token`, defaultHttpOptions()).pipe(
      tap((loginResponse: LoginResponse) => {
          this.setUserFromJwtToken(loginResponse.token, loginResponse.refreshToken, true);
        }
      ));
  ...
  public logout(captureLastUrl: boolean = false, ignoreRequest = false) {
    ...
    if (!ignoreRequest) {
      this.http.post(`${tbApiBaseUrl}/api/auth/logout`, null, defaultHttpOptions(true, true))
        .subscribe(() => {
            this.clearJwtToken();
          },
          () => {
  ...
  public loadOAuth2Clients(): Observable<Array<OAuth2ClientLoginInfo>> {
    const url = `${tbApiBaseUrl}/api/noauth/oauth2Clients?platform=${PlatformType.WEB}`;
  ...
  public getAvailableTwoFaLoginProviders(): Observable<Array<TwoFaProviderInfo>> {
    return this.http.get<Array<TwoFaProviderInfo>>(`${tbApiBaseUrl}/api/auth/2fa/providers`, defaultHttpOptions()).pipe(
      catchError(() => of([])),
      tap((providers) => {
  ...
  private loadUser(doTokenRefresh): Observable<AuthPayload> {
    ...
    if (!authUser) {
      ...
      if (publicId) {
        ...
        return this.http.post<LoginResponse>(this.apiUrlService.getApiUrl('/api/auth/login'), loginRequest, defaultHttpOptions()).pipe(
  ...
  private loadSystemParams(): Observable<SysParamsState> {
    return this.http.get<SysParams>(`${tbApiBaseUrl}/api/system/params`, defaultHttpOptions()).pipe(
      map((sysParams) => {
  ...
  public refreshJwtToken(loadUserElseStoreJwtToken = true): Observable<LoginResponse> {
    ...
    if (this.refreshTokenSubject === null) {
        ...
        if (!refreshTokenValid) {
          ...
          const refreshObservable = this.http.post<LoginResponse>(this.apiUrlService.getApiUrl('/api/auth/token'), refreshTokenRequest, defaultHttpOptions());
